...
import { uploadPDF, getMyDocs, deleteDoc } from "../api/axios";
  ...
  return (
    <div style={styles.page}>
      ...
      <aside style={styles.sidebar}>
        ...
        <button style={styles.navBtn} onClick={() => navigate("/chat")}>💬 Chat</button>
      ...
      <main style={styles.main}>
        ...
        {/* Message */}
        {message && (
          <div style={{ ...styles.msg, background: message.type === "error" ? "#450a0a" : "#052e16" }}>
            {message.text}
          </div>
        )}
...
const styles = {
  page:      { display: "flex", minHeight: "100vh", background: "#0f172a", color: "#f1f5f9" },
  sidebar:   { width: 220, background: "#1e293b", padding: "24px 16px", display: "flex", flexDirection: "column", gap: 8 },
  logo:      { color: "#f1f5f9", marginBottom: 24, fontSize: 18 },
  ...
  dropZone:  { border: "2px dashed #334155", borderRadius: 16, padding: "60px 40px", textAlign: "center", cursor: "pointer", transition: "all 0.2s", marginBottom: 20 },
  dragging:  { borderColor: "#3b82f6", background: "#1e3a5f" },
  dropIcon:  { fontSize: 48, margin: "0 0 12px" },
  dropText:  { fontSize: 16, color: "#94a3b8" },
  dropSub:   { fontSize: 13, color: "#475569", marginTop: 6 },
  msg:       { padding: "12px 16px", borderRadius: 8, marginBottom: 20, color: "#f1f5f9", fontSize: 14 },
  sectionTitle: { fontSize: 18, marginBottom: 16, color: "#94a3b8" },
  empty:     { color: "#475569" },
  docGrid:   { display: "flex", flexDirection: "column", gap: 12 },
  docCard:   { display: "flex", alignItems: "center", gap: 14, background: "#1e293b", borderRadius: 10, padding: "14px 18px" },
  docIcon:   { fontSize: 24 },
  docInfo:   { flex: 1 },
  docName:   { margin: 0, fontWeight: 600, fontSize: 14 },
  docMeta:   { margin: 0, color: "#64748b", fontSize: 12, marginTop: 4 },
  deleteBtn: { background: "transparent", border: "none", cursor: "pointer", fontSize: 18, padding: 4 }
};